...
| Подключить существующий | Импорт распознанной установки Xrayebator по SSH-ключу через read-only `xrayebator inspect --json`; частичные установки сохраняются с диагностикой без автоисправления |
...
Выбранный приватный ключ сохраняется в системном keychain через `keytar` и повторно используется при следующих SSH-операциях и после перезапуска приложения; в карточке сервера хранятся только credential id и отображаемое имя ключа. SSH-пароли, sudo-пароли и passphrase не сохраняются; passphrase зашифрованного ключа вводится заново. Если системный keychain недоступен, plaintext-фолбека нет: ключ остаётся в памяти main process до завершения текущего сеанса, а GUI предупреждает, что после перезапуска его нужно выбрать снова. Также локально сохраняются `subscription_url`, ссылки `vless://` и закреплённый SSH host-key fingerprint. Это bearer/client credentials: защищайте локальные данные и при утечке отзывайте подписку через терминальный workflow.
