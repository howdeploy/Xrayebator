...
- При открытии Server Settings автоматически подключаться по сохранённым credentials и показывать сразу профильную панель; форма доступа — только при отсутствии секрета или ошибке подключения.
...
- при наличии сохранённого credential страница подключается автоматически и показывает профильную панель без формы доступа; сменить доступ можно с карточки сервера в дашборде.
...
В settings после подключения показать только профильную панель; таблицу «Состояние установки» и статус-бокс доступа не показывать (сводка доступа — на карточке сервера). Не делать автоматических repair calls. Existing buttons update/uninstall remain explicit actions.
