...
Если системное хранилище недоступно, plaintext-файл не создаётся: ключ хранится только в памяти main process до выхода из приложения, а GUI предупреждает, что после перезапуска его понадобится выбрать снова. SSH-пароли, sudo-пароли и passphrase ключей не сохраняются; passphrase зашифрованного ключа вводится заново при необходимости. `electron-store` хранит карточку и настройки сервера, credential id/имя ключа, диагностику, URL подписки, VLESS-ссылки (bearer/client credentials) и SHA-256 host-key pin после первого успешного подключения (TOFU). Защищайте локальные данные приложения; при утечке URL или VLESS-ссылок отзовите подписку. При удалении последней карточки, ссылающейся на credential, запись keychain удаляется; если на неё ссылается другая карточка, запись сохраняется. Несовпадение host-key fingerprint блокирует команды; после осознанной переустановки VPS pin можно явно сбросить в Server settings.
