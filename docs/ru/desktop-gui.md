...
Server settings сначала аутентифицируется по SSH. Если у карточки уже есть SSH-пароль или persistent-ключ в системном keychain, страница подключается автоматически; после успеха форма доступа сворачивается в компактный статус «доступ подтверждён» с явным действием «Изменить доступ». После подключения страница позволяет:
