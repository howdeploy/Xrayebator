...
- [x] **4 типа транспорта:** TCP+Vision, TCP+uTLS (Firefox), gRPC, XHTTP, tcp-xudp
...
- [x] **Блокировка рекламы** на уровне маршрутизации
- [x] **Все нужные порты откроются** автоматически
- [x] **Автоматическая подборка настроек** (функция в бете)
- [ ] **Из коробки работает с AdsPower**
...
7. Автоподбор рабочей связки (BETA) - может за вас собрать профиль
...
> Связи с новостями о поломке DNS из-за блокировок VPN провайдерами, рекомендуется на своих клиентах подправить настройки DNS и под рукой иметь список альтернативных ссылок. Если соединение со всем интернетом пропадет, проблема может быть в этом.
...
Да! Создавайте сколько угодно и добавляйте к себе в клиент, переключаясь между ними. Главное делать с разными портами, маршрутами и так далее. Иначе в этом не будет смысла.
...
4. Включить автоподбор, он сделает тоже самое
...
`sudo curl -fsSL https://raw.githubusercontent.com/howdeploy/Xrayebator/experimental/update.sh \
 ...
 #### Использовал автоподбор рабочей связки и не помогло

Это БЕТА, а я вайбкодер. А еще для вашего провайдера/региона все может быть крайне печально. В самых критических ситуациях смотрите в сторону трафика через ssh или второго VPS внутри РФ до вашего зарубежного.
